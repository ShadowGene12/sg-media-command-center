...
interface AuthContextValue {
  ...
  signUp: (
    email: string,
    password: string,
    meta?: { first_name?: string; last_name?: string; business_name?: string },
    ...
    if (!existing || existing.length === 0) {
      ...
      > = {
        sales: [
          ...
          {
            title: "Script your 3 most common objections",
            pillar: "Sales & Conversion",
            pillar_color: "#1D9E75",
          },
          {
        ...
        profit: [
          {
            title: "Calculate gross and net margin per service line",
            pillar: "Profit Optimization",
            pillar_color: "#F59E0B",
          },
          {
            title: "Identify your 3 highest-margin clients and why",
            pillar: "Profit Optimization",
            pillar_color: "#F59E0B",
          },
        ],
        finance: [
          {
            title: "Set up a 90-day rolling cash flow forecast",
      ...
      if (acts.length) {
        await supabase
          .from("action_items")
          .insert(
            acts.map((a) => ({ ...a, user_id: userId, status: "not_started" })),
          );
      }
    }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // ⚡ Bolt: Destructured store actions specifically to avoid Context provider re-renders on every store update
