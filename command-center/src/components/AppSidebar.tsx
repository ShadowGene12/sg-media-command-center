...
export const AppSidebar = () => {
  ...
  const tier = useCommandStore((state) => state.tier); // Bolt: Optimized to prevent unnecessary re-renders of the application shell
        ...
        <div className="flex items-center gap-4 w-full">
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                <h3 className="text-sm font-display font-medium text-white truncate">
                  {displayName}
                </h3>
                <p className="text-[11px] font-mono text-slate-500 truncate tracking-wide uppercase">
                  {tierLabel} TIER
                </p>
              </motion.div>
            )}
          ...
          <div key={i} className="mb-8 px-4">
            ...
            <nav className="space-y-1.5 flex flex-col items-center">
              {group.items.map((item, j) => {
                ...
                const isExactMatch =
                  item.href === "/" ? location.pathname === "/" : isActive;
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          ...
                          {item.locked && (
                            <PremiumBadge className="ml-auto px-1.5 py-0.5 text-[8px]">
                              PRO
                            </PremiumBadge>
                          )}
                        </motion.div>
