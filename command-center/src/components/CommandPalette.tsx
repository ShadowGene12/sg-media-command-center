...
const CMD_ITEM =
  "group flex items-center justify-between px-3 py-2.5 mt-0.5 text-sm rounded-xl cursor-pointer aria-selected:bg-violet-500/10 aria-selected:text-white text-white/70 transition-colors";
  ...
  const { data: sops = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("sops")
        .select("slug, title, pillar, category");
      return data ?? [];
    },
    staleTime: 5 * 60 * 1000,
    ...
    >
      ...
      <Command
        ...
        style={{
          ...
          boxShadow:
            "0 0 80px -10px rgba(109,74,230,0.5), 0 40px 80px rgba(0,0,0,0.8)",
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 border-b border-white/[0.06]">
          ...
          <button
            onClick={() => setCommandPaletteOpen(false)}
            className="p-1.5 text-white/25 hover:text-white/60 transition-colors flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/50 rounded-md"
