...
import { ActionItem, FALLBACK_ACTIONS } from "./ActionPlan";
...
const HomeDashboard = () => {
  useOnboarding("diy");
  // ⚡ Bolt: Split into specific state selectors to prevent HomeDashboard re-renders during command palette or action toggling
  ...
  const timeOfDay =
    new Date().getHours() < 12
      ...
      : new Date().getHours() < 17
        ...
        : "Good evening";
  const firstName =
    profile?.first_name || user?.email?.split("@")[0] || "Shadow";
  ...
  // ── Fetch action items ───────────────────────────────────────
  const { data: dbActions, isLoading: actionsLoading } = useQuery<ActionItem[]>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .limit(6);
      if (error) {
        console.warn("HomeDashboard actions query failed (using fallback):", error.message);
        return null;
      }
      return data ?? [];
    },
  ...
  const actions = dbActions && dbActions.length > 0 ? dbActions : FALLBACK_ACTIONS.slice(0, 3);
      ...
      {isTrialActive && (
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-1.5 h-1.5 rounded-full animate-pulse ${isTrialExpiring ? "bg-amber-400" : "bg-violet-400"}`}
            />
            <span
              className={`font-mono text-xs ${isTrialExpiring ? "text-amber-400" : "text-violet-400"}`}
            >
              {trialDay === 7
                ? "Trial ends tonight — your report stays free forever"
                : `Day ${trialDay} of 7 — full access active`}
            </span>
          </div>
          {isTrialExpiring && (
            <Link to="/upgrade">
              <Button
                size="sm"
                className="h-7 px-3 text-xs bg-violet-600 hover:bg-violet-500 text-white rounded-lg shadow-[0_0_12px_rgba(109,74,230,0.3)]"
              >
                Unlock Operator
              </Button>
            </Link>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-xs text-slate-500">
              Trial ended · Your report is still yours · Fix path requires
              Operator
            </span>
          </div>
          <Link to="/upgrade">
            <div className="animated-laser-border rounded-full p-px">
              <div className="bg-[#050505] rounded-full px-3 py-1 text-[10px] font-mono text-violet-400 font-semibold">
                Upgrade
              </div>
            </div>
          </Link>
        ...
        >
          ...
          <motion.div variants={itemVars} className="md:col-span-8">
            <PremiumCard
              ...
              className="h-full min-h-[300px] p-8 sm:p-10 flex flex-col justify-between group"
            >
              ...
              <div className="relative z-10 space-y-6 flex-1">
                {activeDiagnostic ? (
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-base max-w-xl leading-relaxed font-light">
                        ...
                        <span className="font-mono text-violet-400">
                          {activeDiagnostic.pillar_scores?.[
                            activeDiagnostic.primary_pillar ?? "sales"
                          ] ?? "—"}
                        </span>{" "}
                        / 5.0 · Your diagnostic is ready. Explore your full
                        report to map the fix path.
                      </p>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div>
                      <h3 className="text-3xl font-display font-light text-white mb-3">
                        Take the Bottleneck Detector
                      </h3>
                      <p className="text-slate-400 text-base max-w-xl leading-relaxed font-light">
                        15 questions. 4 minutes. Your full 5-pillar diagnostic
                        mapped and ready inside Command Center.
                      </p>
                    </div>
                  </>
                )}
          ...
          >
            <PremiumCard
              ...
              className="p-8 flex-1 flex flex-col justify-center items-center text-center"
            >
              <p className="text-sm font-mono text-slate-400 uppercase tracking-widest mb-5">
                System Score
              </p>
              <CircularGauge value={overallScore} max={25} />
              <p className="text-xs text-slate-500 mt-4">
                {activeDiagnostic?.created_at
                  ? `Diagnostic: ${new Date(activeDiagnostic.created_at).toLocaleDateString()}`
                  : activeDiagnostic
                    ? "Diagnostic ready"
                    : "No diagnostic yet"}
              </p>
            ...
            <PremiumCard glowColor="rgba(255,255,255,0.05)" className="p-8">
              ...
              {actions.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-slate-500 text-sm font-light">
                    No actions yet — take the Bottleneck Detector to generate
                    your action plan.
                  </p>
                  <Link
                    to="/detector/flow"
                    className="mt-4 inline-block text-xs text-violet-400 hover:text-violet-300 font-mono transition-colors"
                  >
                    Take the Detector →
                  </Link>
                </div>
                ...
                <div className="space-y-1">
                  {actions.slice(0, 3).map((item) => (
                    ...
                    >
                      <button
                        className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${item.status === "completed" ? "bg-emerald-500/20 border-emerald-500/50" : "border-slate-600 hover:border-violet-400"}`}
                      >
                      ...
                      <span
                        className={`text-sm flex-1 font-light ${item.status === "completed" ? "line-through text-slate-500" : "text-slate-200"}`}
                      >
                        {item.title}
                      </span>
              ...
              {actions.length > 0 && (
                <div className="mt-4 pt-4 border-t border-white/[0.04]">
                  <Link
                    to="/actions"
                    className="text-xs text-slate-600 hover:text-violet-400 font-mono transition-colors"
                  >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[9px] font-mono uppercase tracking-widest text-violet-400">
                        Day 3 Unlock
                      </span>
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-400 font-mono border border-violet-500/20">
                        New
                      </span>
                    </div>
                    <h3 className="text-base font-display font-medium text-white mb-1">
                      Full Bottleneck Chain — Secondary Analysis
                    </h3>
                    <p className="text-sm text-slate-400 leading-relaxed mb-3 font-light">
                      Your report now shows the full bottleneck chain across all
                      5 pillars with root-cause mapping. Most operators find
                      their real bottleneck isn't the obvious one.
                    </p>
                    <Link to="/pillars">
                      <Button
                        size="sm"
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 font-mono border border-amber-500/20">
                        Live
                      </span>
                    </div>
                    <h3 className="text-base font-display font-medium text-white mb-1">
                      Your 30-Day Operator Roadmap
                    </h3>
                    <p className="text-sm text-slate-400 leading-relaxed mb-3 font-light">
                      Personalized 30-day fix sequence based on your bottleneck
                      chain. Weekly priorities, SOPs to deploy, and metrics to
                      track.
                    </p>
                    <Link to="/pathways">
                      <Button
          ...
          {dailyInsight && isTrialActive && (
            <motion.div variants={itemVars} className="md:col-span-12">
              <PremiumCard
                ...
                className="p-6 border-white/[0.05]"
              >
                <div className="flex items-start gap-4">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                    style={{
                      backgroundColor: dailyInsight.color + "15",
                      border: `1px solid ${dailyInsight.color}25`,
                    }}
                  >
                    <Lightbulb
                      className="w-5 h-5"
