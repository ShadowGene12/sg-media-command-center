...
const SLASH_COMMANDS = [
  {
    ...
    desc: "Generate strategy based on context",
  },
  {
    id: "task",
    icon: CheckSquare,
    label: "Add Action Item",
    desc: "Create a new execution task",
  },
  {
    id: "kpi",
    icon: BarChart3,
    label: "Log KPI Metric",
    desc: "Update your tracked metric",
  },
];
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900/50 p-6 rounded-2xl border border-white/5 backdrop-blur-xl">
        <div>
          ...
          <h1 className="text-3xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-400">
            Q3 Outbound Engine
          </h1>
        </div>
          ...
          <Card className="bg-slate-900/40 border-white/5 shadow-xl h-[600px] flex flex-col relative overflow-visible">
            <CardHeader className="pb-3 border-b border-white/5 bg-slate-900/50 flex flex-row items-center justify-between">
              <CardTitle className="text-lg flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-indigo-400" /> Notes &
                Ideation
              </CardTitle>
            ...
            <CardContent className="p-0 flex-1 relative">
              ...
              {isGenerating && (
                <div className="absolute inset-x-6 bottom-6 flex items-center gap-3 bg-slate-900/80 backdrop-blur-sm border border-indigo-500/30 rounded-xl p-4 animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <span className="text-xs font-semibold text-indigo-300 uppercase tracking-widest ml-2">
                    Generating
                  </span>
                </div>
              )}
                  ...
                  <div className="py-1">
                    {filteredCommands.length > 0 ? (
                      filteredCommands.map((cmd, i) => (
                        ...
                        >
                          <div
                            className={`p-1.5 rounded-md ${i === selectedIndex ? "bg-indigo-500/20 text-indigo-400" : "bg-slate-700 text-slate-300"}`}
                          >
                            <cmd.icon className="w-4 h-4" />
                          </div>
                          <div>
                            <div
                              className={`text-sm font-medium ${i === selectedIndex ? "text-indigo-300" : "text-slate-200"}`}
                            >
                              {cmd.label}
                            </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-slate-900/40 border-white/5">
            ...
            <CardContent className="p-4 space-y-2">
              <Link
                to="/library/sop-3"
                className="flex items-center justify-between p-2 rounded-md bg-white/5 hover:bg-white/10 transition-colors text-xs text-slate-300"
              >
