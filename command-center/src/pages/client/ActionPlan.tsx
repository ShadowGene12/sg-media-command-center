...
export default function ActionPlan() {
  ...
  const next30 = ACTION_ITEMS.filter((a) => a.status !== "Completed").slice(
    0,
    4,
  );
      ...
      >
        {statusCounts.map(({ label, count }) => {
          ...
          return (
            ...
            >
              <p className={cn("text-2xl font-display font-light", cfg.text)}>
                {count}
              </p>
            ...
            <PremiumCard glowColor="rgba(255,255,255,0.05)">
              {filtered.length === 0 ? (
                ...
                filtered.map((item, i) => {
                  ...
                  const status =
                    STATUS_CONFIG[item.status] ?? STATUS_CONFIG["Not Started"];
                  const priority =
