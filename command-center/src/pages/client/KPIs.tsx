...
function CountUp({ value, color }: { value: string; color: string }) {
  ...
  useEffect(() => {
    ...
    const ctrl = animate(motionVal, parsed.number, {
      duration: 1.2,
      ease: "easeOut",
      delay: 0.3,
    });
  ...
  if (isNaN(parsed.number)) {
    return (
      <span
        className="text-2xl font-display font-light tracking-tight"
        style={{ color }}
      >
        {value}
      </span>
    ...
    <div className="space-y-10 max-w-7xl pb-12">
            ...
            <motion.div key={kpi.label} variants={itemVars}>
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`text-xs font-mono font-semibold ${kpi.trend === "up" ? "text-emerald-400" : "text-red-400"}`}
                  >
                    {kpi.change}
                  </span>
                </div>
            ...
            <PremiumCard glowColor={`${group.color}1A`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-white/[0.03]">
                {group.kpis.map((kpi) => (
                  <div
                    key={kpi.label}
                    className="p-5 bg-black/20 group hover:bg-white/[0.02] transition-colors duration-200"
                  >
                    ...
                    <div className="flex items-end justify-between">
                      ...
                      <div className="flex items-center gap-1 mb-0.5">
                        ...
                        <span
                          className={`text-sm font-mono font-semibold ${kpi.trend === "up" ? "text-emerald-400" : "text-red-400"}`}
                        >
                          {kpi.change}
                        </span>
                      </div>
      ...
      >
        ...
        >
          ...
          <div>
            <p className="text-xs font-mono text-violet-400/80 uppercase tracking-widest mb-2">
              SG Media Interpretation
            </p>
            <p className="text-sm text-slate-400 leading-relaxed">
              Overall performance trajectory is positive. Acquisition metrics
              are strengthening with lead volume up 12% and qualified rate
              improving. The primary area needing attention is close rate —
              currently at 22% against a 25% target. CAC reduction is on track.
              Recommend continued focus on sales enablement and follow-up
              optimization to bridge the conversion gap. Financial indicators
              are stable with positive cash flow momentum.
            </p>
          </div>
        </PremiumCard>
      </motion.div>
