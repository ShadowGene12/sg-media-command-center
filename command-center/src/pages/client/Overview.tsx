...
export default function Overview() {
  const activePriorities = ACTION_ITEMS.filter(
  ...
  const latestReport = REPORTS.filter((r) => r.status === "Completed").sort(
  ...
  const summaryCards = [
    {
      label: "Health Score",
      value: `${ORGANIZATION.healthScore}`,
      sub: "/ 100",
      icon: Target,
      accent: "#10B981",
      progress: ORGANIZATION.healthScore,
    },
    {
      label: "Active Actions",
      value: `${activePriorities.length}`,
      sub: `of ${ACTION_ITEMS.length} total`,
      icon: Map,
      accent: "#6D4AE6",
    },
    {
  ...
  return (
    <div className="space-y-10 max-w-7xl pb-12">
      ...
      <motion.div
        ...
        className="space-y-3"
      >
        <p className="text-xs font-mono tracking-widest text-violet-400/80 uppercase">
          Client Portal — DFY Tier
        </p>
        ...
        <p className="text-base text-slate-400 font-light max-w-2xl leading-relaxed">
          {ORGANIZATION.engagementSummary}
        </p>
        <div className="flex flex-wrap gap-2 pt-1">
          {[
            ORGANIZATION.program,
            ORGANIZATION.industry,
            ORGANIZATION.currentPhase,
          ].map((tag) => (
            <span
              key={tag}
              className="text-[10px] font-mono px-3 py-1 rounded-full border border-white/10 text-slate-500 bg-white/[0.02]"
            >
              {tag}
            </span>
          ))}
        ...
        {summaryCards.map((card) => (
          <motion.div key={card.label} variants={itemVars}>
            <GlassCard className="p-5 flex flex-col gap-3 hover:border-white/[0.12] transition-all duration-300">
              ...
              {card.text && card.sub && (
                <p className="text-xs font-mono text-slate-600">{card.sub}</p>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <GlassCard className="overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.04]">
              ...
              <Link
                to="/client/action-plan"
                className="text-xs font-mono text-slate-600 hover:text-violet-400 transition-colors flex items-center gap-1"
              >
              ...
              {topKpis.map((kpi) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className="text-xl font-display font-light mt-1"
                      style={{ color }}
                    >
                      {kpi.value}
                    </p>
        ...
        <div className="space-y-5">
          ...
          <GlassCard className="overflow-hidden">
            ...
            <div className="p-4 space-y-2">
              {PILLARS.filter((p) =>
                ORGANIZATION.pillarFocus.includes(p.id),
              ).map((p) => {
            ...
            <div className="p-4 space-y-2">
              {topRecs.map((rec) => {
                ...
                return (
                  <div
                    key={rec.id}
                    className="p-3 rounded-xl border border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.03] transition-colors"
                  >
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span
                        className={`text-[9px] font-mono font-semibold px-2 py-0.5 rounded-full border ${URGENCY_COLORS[rec.urgency] ?? ""}`}
                      >
                        {rec.urgency}
                      </span>
                      <div
              ...
              <Link
                ...
                className="text-[10px] font-mono text-slate-600 hover:text-violet-400 transition-colors flex items-center gap-1 pt-1"
              >
