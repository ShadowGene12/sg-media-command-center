...
const STATUS_CONFIG: Record<
  string,
  { text: string; bg: string; border: string; icon: typeof FileText }
> = {
  Completed: {
    ...
    <div className="space-y-10 max-w-6xl pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {REPORTS.map((report, i) => {
          ...
          const status =
            STATUS_CONFIG[report.status] ?? STATUS_CONFIG.Scheduled;
              ...
              <PremiumCard glowColor={`${color}0D`} className="group">
                ...
                <div className="p-5 flex flex-col gap-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div
                      className={`flex items-center gap-1.5 flex-shrink-0 text-[9px] font-mono font-semibold px-2.5 py-1 rounded-full border ${status.text} ${status.bg} ${status.border}`}
                    >
