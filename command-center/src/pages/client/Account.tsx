    ...
    <div className="space-y-8 max-w-4xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Organization profile, engagement details, and workspace settings.
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Badge className="bg-status-success/10 text-status-success border-0 mt-1">
                {ORGANIZATION.status}
              </Badge>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="text-xl font-bold text-foreground">
                  {ORGANIZATION.healthScore}/100
                </span>
                <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                  <div
                    className="h-full rounded-full bg-status-success"
                    style={{ width: `${ORGANIZATION.healthScore}%` }}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {PILLARS.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`flex items-center gap-3 p-3 rounded-lg border ${active ? "bg-muted/50" : "opacity-50"}`}
                >
                  <div
                    className="h-3 w-3 rounded-full"
                    style={{ backgroundColor: `hsl(var(--pillar-${p.id}))` }}
                  />
