...
export const FALLBACK_ACTIONS: ActionItem[] = [
  {
    id: "act-1",
    title: "Draft outbound cold email sequence for VP of Sales",
    status: "in_progress" as ActionStatus,
    created_at: new Date().toISOString(),
  },
  {
    id: "act-2",
    title: "Finalize ICP documentation and value matrix",
    status: "not_started" as ActionStatus,
    created_at: new Date(Date.now() - 86400000).toISOString(),
  },
  {
    id: "act-3",
    title: "Review last month's ad spend against CAC target",
...
const ActionPlan = () => {
  ...
  // Load from Supabase
  const { data: dbActions, isLoading } = useQuery<ActionItem[]>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.warn("Action Items query failed (using fallback):", error.message);
        return null;
      }
      return data ?? [];
    },
  ...
  const actions = dbActions && dbActions.length > 0 ? dbActions : FALLBACK_ACTIONS;
  ...
  const completedPct =
    actions.length > 0
      ? Math.round((counts.completed / actions.length) * 100)
      : 0;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {(
          Object.entries(STATUS_CONFIG) as [
            ActionStatus,
            (typeof STATUS_CONFIG)[ActionStatus],
          ][]
        ).map(([status, cfg]) => {
          const Icon = cfg.icon;
          return (
      ...
      {actions.length > 0 && (
        <PremiumCard
          glowColor="rgba(109,74,230,0.08)"
          className="px-6 py-4 flex items-center gap-4"
        >
          <div className="flex-1">
            <div className="flex justify-between text-xs font-mono text-slate-500 mb-2">
              <span>
                {counts.completed} of {actions.length} actions complete
              </span>
            ...
            <div className="h-1.5 bg-white/[0.05] rounded-full overflow-hidden">
              <motion.div
                ...
                style={{
                  background: "linear-gradient(90deg, #6D4AE6, #00D2FF)",
                }}
                initial={{ width: 0 }}
      ...
      {adding && (
        ...
        >
          <PremiumCard className="p-4 flex gap-3">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && newTitle.trim())
                  addAction.mutate(newTitle.trim());
            ...
            <button
              onClick={() =>
                newTitle.trim() && addAction.mutate(newTitle.trim())
              }
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl text-slate-500 hover:text-white text-xs font-mono"
            >
              Cancel
            </button>
          </PremiumCard>
      ...
      {isLoading ? (
        ...
        <PremiumCard className="p-12 text-center">
          ...
          <p className="text-slate-600 text-xs font-mono mt-1">
            Take the Bottleneck Detector to auto-generate your action plan, or
        ...
        <div>
          <SectionLabel className="mb-5">
            All Actions — {actions.length} items
          </SectionLabel>
          <PremiumCard
            glowColor="rgba(255,255,255,0.04)"
          ...
          >
            {actions.map((item: any, i: number) => {
              const cfg =
                STATUS_CONFIG[item.status as ActionStatus] ??
              ...
              return (
                ...
                >
                  <button
                    className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center transition-all ${item.status === "completed" ? "bg-emerald-500/20 border-emerald-500/40" : "border-slate-700 group-hover:border-violet-400/50"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-light leading-snug ${item.status === "completed" ? "line-through text-slate-500" : "text-slate-200"}`}
                    >
                      {item.title}
                    </p>
                    {item.pillar && (
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 text-[10px] font-mono px-2 py-1 rounded-full border ${cfg.bg} ${cfg.border} ${cfg.text}`}
                    >
