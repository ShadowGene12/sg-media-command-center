...
const PILLARS = [
  ...
  {
    label: "Financial & Performance Control",
    value: "finance",
    color: "#D85A30",
  },
];
...
const TEMPLATES = [
  {
    pillar: "sales",
    title: "Fix Sales Conversion Engine",
    objective:
      "Increase close rate from current to target by implementing a documented sales process and follow-up cadence.",
    start: 0,
    target: 35,
  },
  {
    pillar: "acquisition",
    title: "Build Predictable Lead Engine",
    objective:
      "Establish a consistent outbound channel producing qualified leads weekly with tracked CAC.",
    start: 0,
    target: 20,
  },
  {
    pillar: "market_offer",
    title: "Sharpen Offer Positioning",
    objective:
      "Achieve a clear, specific offer that passes the 10-second stranger test.",
    start: 0,
    target: 80,
  },
  {
    pillar: "profit",
    title: "Improve Margin Per Client",
    objective:
      "Increase net margin by identifying and eliminating top three profit leaks.",
    start: 0,
    target: 45,
  },
  {
    pillar: "finance",
    title: "Build KPI Control Cadence",
    objective: "Implement weekly KPI review and 90-day cash flow visibility.",
    start: 0,
    target: 90,
  },
];
...
export default function SprintWizard() {
    ...
    mutationFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          user_id: user.id,
          title: form.title,
          pillar: selectedPillar?.label || form.pillar,
          color: selectedPillar?.color || "#6D4AE6",
          metric_name: form.metric_name || "Primary Metric",
          baseline_value: form.start_metric ? Number(form.start_metric) : 0,
          target_value: form.target_metric ? Number(form.target_metric) : 0,
          current_value: form.start_metric ? Number(form.start_metric) : 0,
        ...
        .single();
      if (error) {
        console.warn("Sprint Insert Error:", error.message);
        throw error;
      }
      return data;
    },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-12">
      <Link
        to="/sprints"
        className="inline-flex items-center text-sm text-slate-500 hover:text-white transition-colors gap-2 font-mono"
      >
      ...
      {/* Progress */}
      <div>
        <p className="font-mono text-xs text-violet-400/70 uppercase tracking-widest mb-2">
          New Sprint
        </p>
        <h1 className="text-3xl font-display font-light text-white tracking-tight mb-6">
          {currentStep.title}
        </h1>
            ...
            <PremiumCard className="p-6 space-y-5">
              ...
              <div className="rounded-xl p-4 border border-violet-500/20 bg-violet-500/[0.06]">
                ...
                <p className="text-sm text-white/70 font-light">
                  {form.title || "Untitled sprint"}
                </p>
                <p className="text-xs text-white/30 mt-1 font-mono">
                  {selectedPillar?.label} · {form.start_metric || "?"} →{" "}
                  {form.target_metric || "?"} · Ends {form.end_date}
                </p>
              </div>
            </PremiumCard>
          )}
