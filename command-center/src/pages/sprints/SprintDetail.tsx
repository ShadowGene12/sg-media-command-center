...
const MOCK_CHECKINS = [
  {
    ...
    date: "Oct 15",
    metric: 22,
    note: "Implemented new follow-up script. Seeing early signs of higher response rate, but team needs more roleplay.",
  },
  {
    week: 1,
    date: "Oct 8",
    metric: 18,
    note: "Rolled out discovery template. Still getting used to it.",
  },
];
...
export default function SprintDetail() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <Link
        to="/sprints"
        className="inline-flex items-center gap-1.5 text-xs font-mono text-slate-500 hover:text-white transition-colors"
      >
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            <span
              ...
              style={{
                color: PILLAR_COLOR,
                backgroundColor: `${PILLAR_COLOR}18`,
                borderColor: `${PILLAR_COLOR}30`,
              }}
            >
              Sales & Conversion
            </span>
          ...
          <h1 className="text-4xl font-display font-light text-white tracking-tight mb-3">
            Increase Sales Close Rate
          </h1>
          <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-slate-500">
            <span className="flex items-center gap-1.5">
              <Target className="w-3.5 h-3.5" /> Pathway: Sales Engine
            </span>
            <span className="flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5" /> Ends: Nov 12
            </span>
          </div>
        </div>
            ...
            <div className="flex justify-between items-end">
              {[
                ...
                <div key={label} className="text-center flex-1">
                  <p
                    className={`font-display font-light tracking-tighter ${muted ? "text-2xl text-slate-500" : "text-5xl"}`}
                    style={!muted ? { color: PILLAR_COLOR } : undefined}
                  >
                    {value}
                  </p>
                  <p
                    className={`text-[10px] font-mono uppercase tracking-widest mt-2 ${muted ? "text-slate-600" : "font-semibold"}`}
                    style={!muted ? { color: PILLAR_COLOR } : undefined}
                  >
                    {label}
                  </p>
                ...
                >
                  ...
                  <div className="mt-4 pt-4 border-t border-white/[0.04] flex items-start gap-3">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                      ...
                      You've improved 4% over baseline. Focus this week on
                      making sure the team isn't rushing the discovery phase
                      just to use the new script.
                    </p>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="space-y-4">
          ...
          >
            <div className="px-5 py-4 border-b border-white/[0.04]">
              <SectionLabel>Sprint Details</SectionLabel>
            </div>
            <div className="p-5 space-y-3 text-sm">
              {[
                ...
                <div
                  key={label}
                  className="flex justify-between py-2 border-b border-white/[0.03] last:border-0"
                >
          ...
          >
            ...
            <h3 className="text-sm font-semibold text-white mb-1.5">
              Need a strategy pivot?
            </h3>
            <p className="text-xs text-slate-500 mb-4 leading-relaxed">
              Discuss your sprint progress and roadblocks with the AI Advisor.
            </p>
            <Link to="/advisor">
