...
export default function AdminDashboard() {
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [users, diags, sops, actions] = await Promise.all([
        ...
        supabase
          .from("diagnostics")
          .select("id, overall_score, created_at, primary_pillar", {
            count: "exact",
          }),
      ...
      return {
        ...
        avgScore: diags.data?.length
          ? (
              diags.data.reduce((s, d) => s + Number(d.overall_score), 0) /
              diags.data.length
            ).toFixed(1)
          : "—",
        completedActions:
          actions.data?.filter((a) => a.status === "completed").length ?? 0,
      };
    },
    staleTime: 30_000,
  });
  ...
  const CARDS = [
    ...
    },
    {
      ...
      value: stats?.diagCount ?? 0,
      icon: Activity,
      color: "#378ADD",
    },
    {
      label: "Avg Growth Score",
      value: stats ? `${stats.avgScore}/25` : "—",
      icon: BarChart3,
      color: "#1D9E75",
    },
    {
      label: "SOPs Published",
      value: stats?.sopCount ?? 0,
      icon: FileText,
      color: "#F59E0B",
    },
  ];
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1 font-light">
          SG Media Command Center — admin overview
        </p>
      </div>
                ...
                <div key={u.id} className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[10px] font-mono text-slate-600">
                      {u.business_name || "No business name"}
                    </div>
                  </div>
                ...
                <div key={d.id} className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-white/80 font-mono">
                      {Number(d.overall_score).toFixed(1)}/25
                    </div>
                    <div className="text-[10px] font-mono text-slate-600">
