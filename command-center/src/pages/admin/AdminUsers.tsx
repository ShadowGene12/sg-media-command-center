...
export default function AdminUsers() {
  const { data: users = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const trialMap = Object.fromEntries(
        (trials ?? []).map((t) => [t.user_id, t]),
      );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: "On Trial",
            value: users.filter((u: any) => u.trial?.tier === "trial").length,
            color: "#378ADD",
          },
          {
            label: "Diagnosed",
            value: users.filter((u: any) => u.diagnostic).length,
            color: "#1D9E75",
          },
          {
            label: "Admins",
            value: users.filter((u: any) => u.is_admin).length,
            color: "#F59E0B",
          },
        ].map((s) => (
          <PremiumCard
            key={s.label}
            glowColor={`${s.color}12`}
      ...
      {/* User table */}
      <PremiumCard
        glowColor="rgba(109,74,230,0.06)"
