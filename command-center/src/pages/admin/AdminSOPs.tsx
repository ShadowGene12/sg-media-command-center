...
export default function AdminSOPs() {
  ...
  const upsert = useMutation({
    mutationFn: async (sop: Partial<SOP>) => {
      if (!sop.slug || !sop.title || !sop.content)
  ...
  const set = (k: keyof SOP, v: unknown) =>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-mono text-xs text-violet-400/70 uppercase tracking-widest mb-1">
              Admin · SOP Manager
            </p>
            <h1 className="text-3xl font-display font-light text-white">
              {editing.id ? "Edit SOP" : "New SOP"}
            </h1>
          </div>
          ...
          <PremiumCard className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={editing.read_time_mins ?? 10}
                  onChange={(e) =>
                    set("read_time_mins", Number(e.target.value))
                  }
                  className="h-10 bg-white/[0.04] border-white/[0.08] text-white text-sm focus-visible:ring-0 focus-visible:border-violet-500/60"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  value={editing.tier_required ?? "starter"}
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              >
                <div
                  className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${editing.is_popular ? "translate-x-5" : ""}`}
                />
              </div>
              <span className="text-sm text-white/60 font-light">
                Mark as recommended (shows first in library)
              </span>
            </label>
          </PremiumCard>
        ...
        <div className="space-y-3">
          <AnimatePresence>
            {sops.map((sop, i) => (
              ...
              >
                <PremiumCard
                  glowColor={`${sop.pillar_color}12`}
                  className="flex items-center gap-4 p-4 group"
                >
                  <div
                    className="w-1.5 h-10 rounded-full flex-shrink-0"
                    style={{ backgroundColor: sop.pillar_color }}
                  />
                    ...
                    {deleting === sop.id ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="text-[10px] font-mono text-red-400 hover:text-red-300 px-2 py-1 rounded-lg hover:bg-red-500/10"
                        >
                          Yes
                        </button>
                        <button
                          onClick={() => setDeleting(null)}
                          className="text-[10px] font-mono text-slate-500 px-2 py-1"
                        >
                          No
                        </button>
                      </div>
