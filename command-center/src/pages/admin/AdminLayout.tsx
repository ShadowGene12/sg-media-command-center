    ...
    <div className="min-h-screen text-[#F8F9FA] flex relative overflow-hidden">
      ...
      <aside className="w-56 border-r border-white/[0.05] bg-black/60 backdrop-blur-xl flex flex-col z-10 flex-shrink-0">
          ...
          {NAV.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <ChevronRight className="w-3 h-3 text-violet-400/50 ml-auto" />
                )}
              </Link>
            );
          })}
        ...
        <div className="border-t border-white/[0.04] p-3 space-y-1">
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2 rounded-xl text-xs text-slate-600 hover:text-white hover:bg-white/[0.03] transition-all font-mono"
          >
