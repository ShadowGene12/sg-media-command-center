...
export const FALLBACK_SOPS: SOP[] = [
  {
    id: "sop-1",
    title: "B2B Outbound Cold Email Cadence",
    category: "SOPs",
    pillar: "Customer Acquisition",
    pillar_color: "#378ADD",
    summary: "A strict 5-step email sequence designed to break through the noise. Includes exact scripts, follow-up timing, and A/B testing protocols.",
    read_time_mins: 8,
    is_popular: true,
    slug: "b2b-outbound-cadence",
    content: "Content locked. Please connect database."
  },
  {
    id: "sop-2",
    title: "The Value Pricing Matrix",
    category: "Frameworks",
    pillar: "Profit Optimization",
    pillar_color: "#F59E0B",
    summary: "Stop billing by the hour. Use this matrix to quantify the exact financial upside you provide and anchor your pricing to value delivered.",
    read_time_mins: 12,
    is_popular: true,
    slug: "value-pricing-matrix",
    content: "Content locked. Please connect database."
  },
  {
    id: "sop-3",
    title: "Sales Discovery Call Architecture",
    category: "Playbooks",
    pillar: "Sales & Conversion",
    pillar_color: "#1D9E75",
    summary: "A structured approach to the first 30 minutes with a prospect. How to uncover the actual pain point rather than just talking about features.",
    read_time_mins: 15,
    is_popular: false,
    slug: "sales-discovery-architecture",
    content: "Content locked. Please connect database."
  },
  {
    id: "sop-4",
    title: "Why Our Last Ad Campaign Failed",
    category: "Learnings",
    pillar: "Customer Acquisition",
    pillar_color: "#378ADD",
    summary: "Post-mortem analysis on a $10k ad spend that yielded zero closed won deals. The breakdown of message-to-market mismatch.",
    read_time_mins: 6,
    is_popular: false,
    slug: "ad-campaign-failure-postmortem",
    content: "Content locked. Please connect database."
  },
  {
    id: "sop-5",
    title: "End-of-Month Financial Reconciliation",
    category: "SOPs",
    pillar: "Financial & Performance Control",
    pillar_color: "#D85A30",
    summary: "Step-by-step checklist to close the books cleanly, ensuring all outstanding invoices are chased and MRR is accurately reported.",
    read_time_mins: 5,
    is_popular: false,
    slug: "eom-financial-reconciliation",
    content: "Content locked. Please connect database."
  },
  {
    id: "sop-6",
    title: "ICP (Ideal Customer Profile) Blueprint",
    category: "Frameworks",
    pillar: "Market & Offer Clarity",
    pillar_color: "#6D4AE6",
    summary: "Define exactly who you serve. If you sell to everyone, you sell to no one. This framework narrows down the TAM to your highest-converting segment.",
    read_time_mins: 10,
    is_popular: true,
    slug: "icp-blueprint",
    content: "Content locked. Please connect database."
...
export default function LibraryHub({
  ...
  const { data: dbSops, isLoading } = useQuery<SOP[]>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.warn("Library query failed (using fallback):", error.message);
        return [];
      }
      return data ?? [];
    },
  ...
  const sopsToUse = dbSops && dbSops.length > 0 ? dbSops : FALLBACK_SOPS;
  const categorySops = category === "All" ? sopsToUse : sopsToUse.filter(s => s.category === category);

  const filtered = categorySops.filter(
    (s) =>
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-10 pb-12 pt-10 px-4 text-center">
        <PremiumCard glowColor="rgba(139, 92, 246, 0.12)" className="p-10 md:p-14 flex flex-col items-center">
          ...
          <h2 className="text-3xl font-display font-light text-white mb-4">Growth Library Locked</h2>
          <p className="text-slate-400 max-w-lg mb-8 leading-relaxed font-light">
            Your free tier includes permanent access to your Bottleneck Report and dashboard. Upgrade to Operator to unlock 50+ SOPs, playbooks, and the AI Advisor to execute your fix path.
          </p>
          <Link to="/upgrade">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                {popular.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <PremiumCard
                        glowColor={`${item.pillar_color}1A`}
                        className="group relative h-full"
                      >
                        <div
                          className="h-0.5 w-full"
                          style={{
                            background: `linear-gradient(to right, ${item.pillar_color}80, transparent)`,
                          }}
                        />
                        <div className="p-5 flex flex-col gap-3 h-full">
                          <div className="flex items-center justify-between">
                            <span
                              className={`text-[10px] font-mono font-semibold px-2.5 py-1 rounded-full border ${badge.text} ${badge.bg} ${badge.border}`}
                            >
                              {item.category.replace(/s$/, "")}
                            </span>
                            <span className="text-[10px] font-mono text-slate-600 flex items-center gap-1">
                              <Clock className="h-3 w-3" />{" "}
                              {item.read_time_mins} min
                            </span>
                          </div>
