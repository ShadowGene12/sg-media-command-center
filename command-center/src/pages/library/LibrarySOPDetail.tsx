...
export default function LibrarySOPDetail() {
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) {
        console.warn("SOP Detail query failed (using fallback):", error.message);
        return null;
      }
      return data;
    },
  ...
  const sop = dbSop ?? FALLBACK_SOPS.find(s => s.slug === slug);
    ...
    return (
      <div className="max-w-4xl mx-auto py-12 text-center space-y-4">
        ...
        <p className="text-slate-500 text-sm">
          This resource may not exist or hasn't been published yet.
        </p>
        <Button
          onClick={() => navigate(-1)}
          variant="ghost"
  ...
  return (
    ...
    >
      ...
      <Link
        to="/library/sops"
        className="inline-flex items-center text-sm text-slate-500 hover:text-white transition-colors font-mono gap-2"
      >
      ...
      <div className="space-y-4 border-b border-white/[0.06] pb-8">
        <div className="flex items-center gap-3 flex-wrap">
          <span
            ...
            style={{
              ...
              backgroundColor:
                (categoryColor[sop.category] ?? sop.pillar_color) + "15",
              borderColor:
                (categoryColor[sop.category] ?? sop.pillar_color) + "30",
            }}
          >
        ...
        <div className="flex items-center gap-4 pt-2">
          ...
          >
            <Bookmark
              className={`w-4 h-4 ${isSaved ? "fill-violet-400 text-violet-400" : ""}`}
            />
            {isSaved ? "Saved" : "Save"}
          </button>
          <Link
            to="/advisor"
            className="flex items-center gap-2 text-sm text-slate-500 hover:text-violet-400 transition-colors font-mono"
          >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <PremiumCard glowColor="rgba(109,74,230,0.08)" className="p-5">
          <h3 className="text-sm font-display font-medium text-white mb-2">
            Ask AI Advisor about this SOP
          </h3>
          <p className="text-xs text-slate-500 mb-4 font-light">
            Get context-specific advice based on your diagnosis and this
            framework.
          </p>
          <Link to="/advisor">
            <Button
              size="sm"
              variant="ghost"
              className="h-8 text-xs text-violet-400 hover:text-violet-300 hover:bg-violet-500/10 border border-violet-500/20 rounded-lg px-3"
            >
              Open AI Advisor →
            </Button>
          </Link>
        ...
        <PremiumCard glowColor="rgba(29,158,117,0.08)" className="p-5">
          ...
          <p className="text-xs text-slate-500 mb-4 font-light">
            Create an action item from this SOP and track implementation.
          </p>
          <Link to="/actions">
            <Button
              size="sm"
              variant="ghost"
              className="h-8 text-xs text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3"
            >
              View action plan →
            </Button>
          </Link>
