    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      ...
      <div className="space-y-6">
        ...
        <p className="text-xl text-slate-400 max-w-2xl leading-relaxed">
          Build a repeatable, trainable sales process to close more qualified
          leads without relying on founder magic.
        </p>
        ...
        <div className="flex gap-6 py-6 border-y border-white/10">
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>
              <strong className="text-white">Goal:</strong> Increase close rate
              by 20%
            </span>
          </div>
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>
              <strong className="text-white">Duration:</strong> 4 Weeks
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4 relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
          {MOCK_STEPS.map((step, index) => (
            ...
            >
              {/* Icon / Node */}
              <div
                className={`flex items-center justify-center w-12 h-12 rounded-full border-4 border-[#0A0A0C] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow shadow-slate-950 z-10 ${
              ...
              {/* Card */}
              <Card
                className={`w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] transition-all ${
                  step.status === "completed"
                    ? "bg-slate-900/50 border-emerald-500/30"
                    : step.status === "active"
                      ? "bg-slate-900 border-primary shadow-[0_0_15px_rgba(139,92,246,0.15)]"
              ...
              >
                <CardContent className="p-6">
                  ...
                  <h3
                    className={`text-lg font-bold mb-4 ${step.status === "locked" ? "text-slate-500" : "text-white"}`}
                  >
                    {step.title}
                  </h3>
                  ...
                  {step.status !== "locked" ? (
                    ...
                    <Button
                      variant="outline"
                      disabled
                      className="w-full border-white/5 text-slate-600"
                    >
                      Locked
                    </Button>
                  )}
