      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 border-b border-white/10 pb-8">
        <div>
          ...
          <h1 className="text-4xl font-display font-bold text-white mb-4">
            Draft the Sales Script
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl">
            Take your documented process and convert it into a line-by-line
            script framework that allows for natural conversation while hitting
            all required psychological checkpoints.
          </p>
        </div>
              ...
              <CardContent className="p-0">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`text-lg ${completedTasks.includes(index) ? "text-slate-500 line-through" : "text-slate-200"}`}
                    >
                      {task}
                    </span>
                  </div>
          ...
          {isStepComplete && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6 text-center animate-in fade-in zoom-in duration-500">
              ...
              <p className="text-emerald-200/70 mb-6">
                You've finished all action items for this step.
              </p>
              <Button
                size="lg"
            ...
            <CardContent className="p-6 space-y-6">
              <div>
                <h3 className="font-semibold mb-2 flex items-center gap-2">
                  <MessageSquare className="w-5 h-5 text-primary" /> AI Advisor
                  Prompts
                </h3>
                <p className="text-sm text-slate-400 mb-4">
                  Stuck on this step? Try asking the AI:
                </p>
                <div className="space-y-2">
                  <button className="w-full text-left p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-sm text-slate-300 transition-colors">
                    "Review my discovery questions to see if they're too
                    interrogational."
                  </button>
                  <button className="w-full text-left p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-sm text-slate-300 transition-colors">
                    "Give me an example of a good transition from discovery to
                    pitch."
                  </button>
                </div>
              </div>
