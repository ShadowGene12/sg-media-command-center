...
const pricingTiers = [
  {
    ...
    features: [
      ...
      <span
        key="no-ai"
        className="text-slate-400/70 flex items-center line-through"
      >
        <X className="h-4 w-4 mr-2" /> AI Advisor
      </span>,
      <span
        key="no-sprint"
        className="text-slate-400/70 flex items-center line-through"
      >
        <X className="h-4 w-4 mr-2" /> Sprint Tracking
      </span>,
    ],
    cta: "Current Plan",
    disabled: true,
    ...
    <div className="min-h-screen bg-[#0B0819] text-white py-20 px-4 flex flex-col items-center">
      <div className="text-center mb-16 max-w-3xl">
        ...
        <p className="text-lg text-slate-400">
          Choose the tier that fits your current operational needs. Upgrade
          anytime as you grow.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-7xl">
        {pricingTiers.map((tier) => (
          <Card
            key={tier.name}
            className={`bg-slate-900/50 border-white/5 border-t-white/10 backdrop-blur-xl relative overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-xl h-full
              ${tier.popular ? "border-primary/50 border-t-primary/60 shadow-[0_0_30px_rgba(139,92,246,0.15)] transform md:-translate-y-2 hover:-translate-y-3 hover:border-primary/60 hover:shadow-[0_0_40px_rgba(139,92,246,0.25)]" : ""}
            `}
          >
            {tier.popular && (
