...
const GREETING: Message = {
  ...
  role: "assistant",
  content:
    "I'm your SG Media AI Advisor. I have access to your bottleneck diagnosis, your SOP library, and your sprint metrics. What do you want to work on today?",
};
...
- Pillar Scores: ${Object.entries(scores)
    .map(([k, v]) => `${k}: ${v}/5`)
    .join(", ")}
...
export default function AIAdvisorHub() {
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      // SECURITY: Do not expose internal infrastructure details or error messages to the user
      const fallbackMsg: Message = {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content: "I'm currently unable to process your request due to a temporary system error. Please try again later.",
      };
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <div
                  ...
                  className={cn(
                    ...
                    i <= credits
                      ? "bg-violet-500 shadow-[0_0_6px_rgba(109,74,230,0.5)]"
                      : "bg-white/10",
                  )}
                />
              ))}
